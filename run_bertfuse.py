...
from model import Bert_CNN_LSTM

# 设置logger
logging.basicConfig(filename='training_bertfuse_lr.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
if __name__ == '__main__':
    ...
    model = Bert_CNN_LSTM(len(labelIndex), criterion).to(device)
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=weight_decay)

    # 使用余弦退火学习率调度器和预热
    total_steps = len(trainDataloader) * epoch
    ...
    for e in range(epoch):
        ...
        logger.info(f'Epoch: {e+1}, Loss: {epochloss:.5f}, Acc: {acc:.4f}, F1: {f1:.4f}, LR: {current_lr:.6f}')

        # 绘图
        plt.plot(epochPlt, trainLossPlt, 'r') 
        plt.plot(epochPlt, devAccPlt, 'g')   
        plt.plot(epochPlt, devF1Plt, 'b')     
        plt.ylabel('Loss/Accuracy/F1')
        plt.xlabel('Epoch')
        plt.legend(['TrainLoss', 'DevAcc', 'DevF1'], loc='best')
        plt.savefig("bert_fuse_lr.png")
