...
from model import Bert_Crf_Model

# 设置logger
logging.basicConfig(filename='training_bertcrf.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
if __name__ == '__main__':
    ...
    model = Bert_Crf_Model(len(labelIndex), criterion).to(device)
    ...
    for e in range(epoch):
        ...
        # 记录日志
        logger.info(f'Epoch: {e+1}, Loss: {epochloss:.5f}, Acc: {acc:.4f}, F1: {f1:.4f}')
